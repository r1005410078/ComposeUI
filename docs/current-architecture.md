...
```text
packages/editor/src/
├── session/           # Session Scope（viewport、selection、gridSize/snap…）+ 坐标 + snap 纯函数
├── canvas/            # mount / board-render / overlay / pointer* / workspace-canvas / preview
├── tree/              # 组件树
...
History                 gridVisible / gridSize     toolbar / output
                        snapEnabled, mode
                        pointer preview draft
```

### 5.2 Session 网格与吸附（会话态，不进文档）

| 字段 | 默认 | 说明 |
| --- | --- | --- |
| `gridVisible` | `true` | 是否绘制工作区网格底景 |
| `gridSize` | `8` | 次网格步长（world/local 同一数值语义）；合法范围 `1…1024` |
| `snapEnabled` | `true` | 是否启用 Free Layout 吸附；与 `gridVisible` **独立** |
...
| 区域 | 路径 | 说明 |
| --- | --- | --- |
| Session | `session/session.ts` | 会话态（含 `gridSize`/`snapEnabled`）；不进文档 |
| 坐标 | `session/coordinates.ts` | screen ↔ world ↔ parent-local、`zoomAt` |
| 吸附 | `session/snap.ts` | 网格量化纯函数；供指针 preview/commit 复用 |
| 交互草稿 | `canvas/interactions.ts`, `canvas/group-resize.ts` | 拖拽/组缩放纯几何，commit 再 dispatch |
| 画布挂载 | `canvas/mount.ts` | `mountEditor`：装配 board / workspace-canvas / overlay / pointer / preview |
| Board 渲染 | `canvas/board-render.ts` | page board、节点 DOM |
| 工作区底景 | `canvas/workspace-canvas.ts` | Canvas2D 无限 world 网格 + 标尺/游标读数 |
| 叠层 | `canvas/overlay.ts` | SVG 选框与手柄 |
| 指针 | `canvas/pointer.ts` | move/resize/group-resize；可选 snap |
| 平移 / 框选 | `canvas/pointer-pan.ts`, `pointer-marquee.ts` | 手势拆分；平移不吸附 |
...
workspace (position: relative)
  ├── workspace-canvas   # Canvas2D underlay，pointer-events: none
  ├── world              # page board + 节点 DOM
  └── overlay            # SVG 选框/手柄
```
...
```text
pointermove/up
  → shouldSnap = snapEnabled && !Alt
  → move:  snapPoint(local origin, gridSize)
  → resize: snapRect(..., gridSize, edgesForHandle)
  → DOM 预览与最终 dispatch 使用同一套 snapped 几何
```

- `gridVisible` **不影响**是否吸附。
- 松手后仍经 `node.move` / `node.resize*` 命令写文档；吸附只改提交坐标，不改写路径。

### 5.6 默认 Workspace 面板（一等公民）
...
- 工作区辅助网格为 **Canvas2D underlay**；画布节点为 **rectangle 的 div**；选框/手柄在 **SVG overlay**。
...
| **Session** | viewport、selection、expanded、`gridVisible`/`gridSize`/`snapEnabled`、mode | 一般不进页面文档；可进 log | `EditorSession` |
...
pointermove  → 可选 snapPoint/snapRect → 更新预览 DOM/叠层（不写 store）
pointerup    → 同一 snap 规则 → commit delta → dispatch({ id: "node.move", … })
