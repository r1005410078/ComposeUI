...
- Document Scope 与 Session Scope 分离；viewport、selection、expanded、hover 和指针预览不进入 canonical 文档。
- 单页面画板与 Free Layout 矩形节点，组件树和 SVG 选区保持同步。
- 指针 move/resize 使用临时预览并在完成时提交一次持久事务，支持 undo/redo。
- 唯一 Vite Playground 提供确定性 M1 场景，并由 Vitest、fast-check、JSON Golden 和 Chromium E2E 覆盖。
...
**状态：已完成。** 当前实现覆盖桌面 Free Layout 编辑器、Session Scope、组件树、SVG 选区、指针 move/resize、undo/redo，以及确定性 Golden、property test 和 Chromium E2E。该状态不包含本节“不在 M1 中”列出的后续能力。
