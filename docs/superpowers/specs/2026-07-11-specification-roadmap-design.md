...
M0 建立了领域和工具链的最小闭环；M1 在此基础上完成了桌面 Free Layout 编辑器骨架：

- Bun Workspace、严格 TypeScript、Oxlint、Oxfmt、Vite、Vitest、Playwright。
- `@composeui/core` 的版本化 `PageDocument`、规范化 `RecordStore`、结构化 `Diagnostic`、原子 Transaction、Command 和有限 History。
- Document Scope 与 Session Scope 分离；viewport、selection、expanded、hover、辅助网格状态和指针预览不进入 canonical 文档。
- Workspace 支持中键平移、以指针为中心的滚轮缩放和可切换的临时辅助网格。
- 单页面画板与 Free Layout 矩形节点，组件树和 SVG 单选、多选、框选覆盖层保持同步。
- 指针 move/resize 使用临时预览并在完成时提交一次持久事务，支持 undo/redo。
- 组件树通过 Core commands 提供重命名、显隐、锁定和同级上下移动；重排一次事务提交且可撤销。
- 唯一 Vite Playground 提供确定性 M1 场景、创建节点和 canonical JSON 导出，并由 Vitest、fast-check、JSON Golden 和 Chromium E2E 覆盖。

M1 仍不是完整产品：Auto Layout/Grid、框架 Adapter 与业务组件、bindings、Figma、Yjs、GPU/Worker、大屏优化和高级矢量编辑均未实现。
...
**状态：已完成。** 当前实现覆盖桌面 Free Layout 编辑器、Session Scope Workspace 平移/缩放/辅助网格、修饰键多选与框选、命令驱动组件树基础操作和同级重排、SVG 选区、指针 move/resize、undo/redo、确定性创建与 canonical JSON 导出，以及 Golden、property test 和 Chromium E2E。该状态不包含本节“不在 M1 中”列出的后续能力。
...
下一步是依据 M2 边界编写独立实施计划，聚焦 Auto Layout、Grid、基础节点和确定性布局 Projection。框架 Adapter/业务组件与 bindings 属于 M3；Figma、Yjs、GPU/Worker、大屏优化和高级矢量仍按后续里程碑推进，不能因 M1 完成而视为已实现。
