...
test("mounts the Godot 2D workspace with the canonical panels and no mode bar", async ({
  ...
  await expect(page.getByTestId("node-search")).toHaveAttribute("placeholder", "检索节点")
  await page.getByTestId("node-search").fill("Blue rectangle")
  await expect(page.getByTestId("tree-row-node-red")).toBeHidden()
  await expect(page.getByTestId("tree-row-node-blue")).toBeVisible()
  await page.getByTestId("node-search").fill("")
  await expect(page.getByTestId("collapse-all")).toHaveAttribute("aria-label", "全部折叠")
  await page.getByTestId("collapse-all").click()
  await expect(page.getByTestId("tree-toggle-page-1")).toHaveAttribute("aria-expanded", "false")
  await expect(page.getByTestId("collapse-all")).toHaveAttribute("aria-label", "全部展开")
  await expect(page.getByTestId("tree-row-node-blue")).toBeHidden()
  await page.getByTestId("collapse-all").click()
  await expect(page.getByTestId("tree-toggle-page-1")).toHaveAttribute("aria-expanded", "true")
  await expect(page.getByTestId("collapse-all")).toHaveAttribute("aria-label", "全部折叠")
